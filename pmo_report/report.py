...
class ReportGenerator:
    ...
    def _ai_generate(self, stats: ProjectStats) -> Dict[str, str]:
        ...
        data = {
            ...
            "risk_items": [
                {"name": ts.task.name, "level": ts.risk_level, "reason": ts.risk_reason}
                for ts in stats.task_stats if ts.risk_level != "正常"
            ],
            "tasks_in_progress": [
                {"name": ts.task.name, "progress": ts.task.progress}
        ...
        try:
            text = ai_module.call_deepseek(
                [
                    {"role": "system", "content": "你是严谨、专业的 PMO 项目周报撰写专家，只输出纯文本。"},
                    {"role": "user", "content": prompt},
                ],
                temperature=0.6,
            )
            overview, next_plan = self._split_ai_output(text)
            return {"overview": overview, "next_plan": next_plan}
    ...
    def render(self, stats: ProjectStats, use_ai: bool = True, today: Optional[date] = None,
               rules: Dict = None) -> Dict[str, str]:
        """生成周报。返回 {report, report_html, report_text, ai_used, error}。"""
    ...
    def render_blocks(self, blocks, stats: ProjectStats, use_ai: bool = True,
                      today: Optional[date] = None, rules: Dict = None) -> Dict[str, str]:
